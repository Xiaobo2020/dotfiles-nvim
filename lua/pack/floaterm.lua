...
function M.setFTToggleMap(key, name, cmd)
    G.map({
        { 'n', key, string.format(":lua require('pack/floaterm').toggleFT('%s', '%s')<cr>", name, cmd), { silent = true, noremap = true }},
...
function M.config()
    ...
    G.map({
        { 'n', '<F5>', ':lua require("pack/floaterm").runFile()<cr>', {silent = true, noremap = true}},
        { 'i', '<F5>', '<esc>:lua require("pack/floaterm").runFile()<cr>', {silent = true, noremap = true}},
